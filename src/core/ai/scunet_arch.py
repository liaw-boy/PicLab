...
import numpy as np
try:
    from thop import profile  # only used by __main__ benchmark; optional
except ImportError:
    profile = None
...
# Inline replacements for timm.layers.{trunc_normal_, DropPath} to avoid
# the timm → torchvision dependency chain (torchvision often mismatches torch versions).
def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
    """Fill tensor with truncated normal distribution. Inline version of timm's."""
    with torch.no_grad():
        size = tensor.shape
...
class DropPath(nn.Module):
    """Stochastic depth per sample. Inline version of timm.layers.DropPath."""
